...
describe("runtime state", () => {
  it("round-trips turn-scoped guard state without clobbering defaults", () => {
    ...
    state.guard = { intent_id: "turn-1", status: "resolved", level: "L2", resolution: "existing_bet", block_issued: true };
    ...
    expect(loaded.guard).toEqual({ intent_id: "turn-1", status: "resolved", level: "L2", resolution: "existing_bet", block_issued: true });
  ...
  it("migrates old runtime files without treating block_issued as authorization", () => {
    const root = fresh();
    writeFileSync(path.join(root, ".product", ".runtime", "session.json"), JSON.stringify({ guard: { level: "L2", resolution: null, block_issued: true } }));
    const loaded = loadRuntime(root);
  ...
  it("does not migrate a legacy build_anyway resolution into authorization", () => {
    const root = fresh();
    writeFileSync(
      path.join(root, ".product", ".runtime", "session.json"),
      JSON.stringify({ guard: { level: "L2", resolution: "build_anyway", block_issued: true } }),
    );
    const loaded = loadRuntime(root);
    expect(loaded.guard.status).toBe("unresolved");
    expect(loaded.guard.resolution).toBeNull();
    expect(loaded.guard.intent_id).toBeNull();
    rmSync(tmp, { recursive: true, force: true });
  });
  ...
  it("bumpMetric and touchHeartbeat persist", () => {
    const root = fresh();
    bumpMetric(root, "guard_blocked"); bumpMetric(root, "guard_blocked"); touchHeartbeat(root, "before_mutation");
    const loaded = loadRuntime(root);
  ...
  it("clearSessionOverrides keeps product-intent state", () => {
    ...
    state.hook_overrides = { before_mutation: { enabled: false } };
    state.active_bet = "DEC-0001"; state.turn_id = "turn-1";
    saveRuntime(root, state); clearSessionOverrides(root);
    const loaded = loadRuntime(root);
