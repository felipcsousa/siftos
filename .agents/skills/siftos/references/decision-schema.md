...
A bet and a decision are the same artifact in different stretches of one
lifecycle (V2):
...
| Status | Meaning |
| --- | --- |
| `draft` | incomplete record / raw idea |
| `shaping` | problem, target user, hypothesis, SVT being formed |
| `validating` | validation contract defined; test running |
| `ready` | shaped and validated; awaiting human decision |
| `proposed` | analysis ready, human decision pending (v0.2 status, still valid) |
| `accepted` | human decision confirmed |
| `building` | bet being implemented |
| `shipped` | decision operationalized |
| `measuring` | outcome being measured |
| `reviewed` | outcome analyzed |
| `rejected` | proposal deliberately rejected |
| `cancelled` | accepted decision no longer executed |
| `paused` | bet temporarily halted |
| `failed` | bet hypothesis disproven |
