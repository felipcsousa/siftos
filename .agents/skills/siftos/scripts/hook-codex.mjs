...
if (!config?.enabled) {
  output(config?.config_error ? { systemMessage: `SiftOS hooks disabled: ${config.config_error}. Run siftos doctor.` } : {});
  process.exit(0);
}
...
try {
  switch (eventName) {
    case "session_start": {
      ...
      output({ hookSpecificOutput: { hookEventName: "SessionStart", additionalContext: buildCapsule(root, loadRuntime(root)) } });
      break;
    }
    case "prompt_submit": {
      ...
      const advisory = next.candidate === "technical"
        ? ""
        : "This turn may contain a product decision. Before the first product-changing mutation, resolve SiftOS Product Guard; retrying a denied mutation is not authorization.";
      output({ hookSpecificOutput: { hookEventName: "UserPromptSubmit", additionalContext: advisory } });
      break;
    }
    case "before_mutation": {
      ...
      if (!result.allowed) {
        ...
        output({ hookSpecificOutput: { hookEventName: "PreToolUse", additionalContext: result.message } });
      } else output({});
      break;
    }
    case "after_mutation": {
