...
export function recordMutation(root, { toolName, toolInput }) {
  ...
  const files = toolStrings(toolInput).filter((value) => /[\\/]|\.[A-Za-z0-9]{1,8}$/.test(value)); for (const file of files) if (!state.mutation.files.includes(file)) state.mutation.files.push(file);
...
function deriveActiveBet(decisions) { const building = decisions.filter((decision) => decision.status === "building"); return building.length === 1 ? building[0] : null; }
export function closeout(root) {
  ...
  if (!bet) { saveRuntime(root, state); return { continue: false, message: "SiftOS closeout advisory: implementation mutations occurred but no unique active building Bet is attached. Run /siftos ship manually when a Bet is known." }; }
